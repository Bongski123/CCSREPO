...
router.get('/top-downloads', async (req, res) => {
    const query = `
      SELECT 
        ...
        r.status,
        r.file_privacy,
        COALESCE(SUM(d.download_count), 0) AS downloadCount,  -- Summing download counts from downloads table
      ...
      GROUP BY r.research_id, r.title, r.abstract, r.publish_date, r.status, r.file_privacy
